...
def generate_dataset(num_samples=200):
    ...
    # 3. Injection d'erreurs d'étiquetage (Mislabeled data) - configuré par NOISE_RATE
    # Cela empêchera d'avoir une précision de 100%
    unique_types = list(df['anomaly_type'].unique())
    flip_mask = np.random.random(len(df)) < NOISE_RATE
    for idx in df[flip_mask].index:
        choices = [t for t in unique_types if t != df.loc[idx, 'anomaly_type']]
        new_type = np.random.choice(choices)
        df.loc[idx, 'anomaly_type'] = new_type
        df.loc[idx, 'label'] = 'ANOMALY' if new_type != 'NONE' else 'NORMAL'
        
    df = df.sort_values(['timestamp', 'sample_id']).reset_index(drop=True)  # Trier par timestamp et sample_id
