...
for i in range(128):
    ...
    pre_payloads.append({'motor_id': mid, 'vib_rms': r1, 'vib_peak': r2, 'temperature': r3, 'status': 'normal'})
...
def worker(wid, queue):
    ...
    for i in range(top):
        s = ns()
        prod.send(tpc, pls[i & 127])
        prod.flush()
        ap(lat, (ns() - s) * 0.000001)
    prod.close()
    queue.put(lat)

print(f'[3/5] Demarrage Kafka Producer ({NUM_WORKERS} workers)...', flush=True)
s = time.time()
q = mp.Queue()
