...
export function InterventionChart({ tab, date }: { tab: string; date: string }) {
  ...
  const preventifPct = total > 0 ? Math.round((preventif?.value || 0) / total * 100) : 0;
          ...
          <>
            ...
            <div className="relative flex items-center justify-center w-40 h-40">
              <svg width="160" height="160" viewBox="0 0 180 180" fill="none">
                <circle cx="90" cy="90" r="72" stroke={correctif?.color || "#0C6CF2"} strokeWidth="25.2" fill="none" opacity="0.2" />
                <path
                  d="M90 18C104.221 18 118.124 22.2114 129.954 30.1029C141.785 37.9945 151.013 49.2127 156.476 62.3428C161.939 75.4728 163.391 89.9265 160.649 103.881C157.908 117.835 151.095 130.665 141.071 140.752C131.046 150.839 118.259 157.732 104.323 160.561C90.3857 163.39 75.9233 162.028 62.7593 156.648"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-[#E6F0F2] text-2xl font-bold">{preventifPct}%</span>
                <span className="text-[#98A6A8] text-[11px] uppercase font-bold tracking-wider mt-1">Préventif</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-6">
              {interventions.map(i => (
                <div key={i.type} className="flex items-center gap-2">
                  ...
                  <span className="text-[#C9E7E6] text-[12px] font-medium">{i.type}</span>
                </div>
              ))}
            </div>
            <p className="text-[#64748B] text-[11px] italic mt-2">Données pour {date}</p>
          </>
        )}
      </div>
    </div>
