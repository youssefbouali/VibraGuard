...
describe("apiRequest hooks", () => {
  ...
  it("useWorkOrders calls the expected endpoint", async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith("GET", "/api/v1/iot/work-orders");
  });
  ...
  it("useMtbfBySite calls the expected endpoint", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockApiRequest).toHaveBeenCalledWith("GET", "/api/v1/bi/mtbf-by-site");
  });
  ...
  it("useMaintenanceCosts calls the expected endpoint", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockApiRequest).toHaveBeenCalledWith("GET", "/api/v1/bi/maintenance-costs");
  });
  ...
  it("useInterventions calls the expected endpoint", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockApiRequest).toHaveBeenCalledWith("GET", "/api/v1/bi/interventions");
  });
  ...
  it("useTraceability calls the expected endpoint", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockApiRequest).toHaveBeenCalledWith("GET", "/api/v1/blockchain/traceability");
  });
});
